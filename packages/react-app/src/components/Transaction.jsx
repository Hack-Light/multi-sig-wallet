...
function App(props) {
  ...
  const apiBaseUrl = "https://multisig-wallet.onrender.com";
  ...
  const neededSigns = useContractReader(readContracts, "MultiSig", "signRequired");
  const MultiSig = readContracts ? readContracts.MultiSig : "";
  ...
  //     let role = await MultiSig.getMemberRole(_members[i]);
  //     newRoles.push(role);
  //     oldRoles = [...newRoles];
  //     setRoles(newRoles);
  //   }
  ...
  async function getMembers() {
    const newMembers = await MultiSig.getSigners();
    setMembers(newMembers);
    // getRole(newMembers);
  }

  useEffect(() => {
    if (MultiSig && userSigner) getMembers();
  }, [MultiSig, userSigner]);

  // useEffect(async () => {
  //   if (userSigner && MultiSig) {
  //     let newMemberRole = await MultiSig.getMemberRole(address);
