...
describe("MetaMultiSigWallet Test", () => {
  ...
  describe("Deployment", () => {
    ...
    it("Multi Sig Wallet should own all the monyo token", async () => {
      let metaMultiSigWalletMonyoBalance = await monyo.balanceOf(
        metaMultiSigWallet.address
      );
  ...
  describe("Testing MetaMultiSigWallet functionality", () => {
    it("Adding a new signer", async () => {
      ...
      let callData = metaMultiSigWallet.interface.encodeFunctionData(
        "addSigner",
        [newSigner, 1]
      );

      let hash = await metaMultiSigWallet.getTransactionHash(
        nonce,
        to,
        value,
        callData
    ...
    it("Allowing addr1 to spend 10 Monyo tokens. Then addr1 transfers the Monyo tokens to addr2", async () => {
      ...
      let callData = monyo.interface.encodeFunctionData("approve", [
        addr1.address,
        amount,
      ]);
      ...
      let metaMultiSigWallet_addr1Allowance = await monyo.allowance(
        metaMultiSigWallet.address,
        addr1.address
      );
